...
from lightly_transforms.random_crop_and_flip_with_grid import RandomResizedCropAndFlip
from lightly_transforms.rotation import random_rotation_transform
from lightly_transforms.utils import IMAGENET_NORMALIZE
...
class BaseCollateFunction(nn.Module):
    ...
    def forward(self, batch: List[Tuple[Image.Image, int, str]]):
        ...
        # list of transformed images. THIS IS WHERE I CHANGED STUFF
        for i in range(len(batch)):
            if len(batch[i][0].size())==2: #Dimensional expansion required
                li_i = list(batch[i])
                li_i[0] = batch[i][0][None,:,:].repeat(3,1,1) #Spanning 3 color channels
                batch[i] = tuple(li_i)
        transforms = [
...
class ImageCollateFunction(BaseCollateFunction):
    ...
    def __init__(
        ...
        #    T.ToTensor(), #REMOVED FOR FLOAT32 FUNCTIONALITY
        ]

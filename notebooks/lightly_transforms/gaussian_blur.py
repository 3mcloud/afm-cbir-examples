...
PIL_img_transform = Compose([PILToTensor()])
...
class GaussianBlur:
    ...
    def __call__(self, sample):
        ...
        if prob < self.prob:
            ...
            #print(type(to_pil_image(sample).filter(ImageFilter.GaussianBlur(radius=sigma))))
            #print('')
            return PIL_img_transform(to_pil_image(sample).filter(ImageFilter.GaussianBlur(radius=sigma))).to(torch.float)
